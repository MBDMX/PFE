...
export default function WorkOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchOrderData = async () => {
      try {
        const data = await gmaoApi.getWorkOrder(id);
        setOrder(data);
      } catch (err) {
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      ...
      default: return { label: status || 'Inconnu', color: 'text-slate-400', bg: 'bg-slate-400/10', icon: Clock };
    }
  };
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <button className="px-6 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm transition-all shadow-lg shadow-blue-500/20 active:scale-95 flex items-center gap-2">
